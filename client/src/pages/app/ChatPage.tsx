...
function MentionChip({
  ...
  file: MentionedFile;
  onClick?: (file: MentionedFile) => void;
}) {
  return (
    <span
    ...
    >
      <span
        ...
        style={{ background: TYPE_BADGE[file.type] || '#737373' }}
      />
...
function MessageBubble({
  ...
  onMentionClick?: (file: MentionedFile) => void;
}) {
  ...
  if (msg.role === 'user') {
    return (
      <div className="flex gap-3 items-end flex-row-reverse group">
        ...
        <div className="px-3.5 py-2.5 rounded-12 leading-[1.65] text-neutral-950 break-words text-para-md bg-neutral-50 shadow-[inset_0_0_0_1px_theme(colors.neutral.200)] rounded-br-4 max-w-full order-1">
          {renderContent()}
        </div>
        <div className="w-[30px] h-[30px] rounded-full flex items-center justify-center shrink-0 bg-neutral-200 text-neutral-600 text-[11px] font-semibold tracking-[0.02em] border-[1.5px] border-neutral-200">
          JB
        </div>
      </div>
    );
  ...
  return (
    <div className="flex gap-3 items-end group">
      ...
      <div className="flex flex-col gap-1 max-w-[78%]">
        <div className="px-3.5 py-2.5 rounded-12 leading-[1.65] text-neutral-950 break-words text-para-md bg-white shadow-[inset_0_0_0_1px_theme(colors.neutral.200)] rounded-bl-4">
          {msg.content}
        </div>
...
export function ChatPage() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (atQuery !== null && filteredFiles.length > 0) {
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setMentionHighlight((i) => (i - 1 + filteredFiles.length) % filteredFiles.length);
        return;
      }
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setMentionHighlight((i) => (i + 1) % filteredFiles.length);
        return;
      }
      if (e.key === 'Enter' || e.key === 'Tab') {
        e.preventDefault();
        insertMention(filteredFiles[mentionHighlight]);
        return;
      }
      if (e.key === 'Escape') {
        setAtQuery(null);
  ...
  const sendMessage = useCallback(
    (text: string, mentions: MentionedFile[]) => {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-white relative">
          ...
          <div className="w-full max-w-[720px] relative flex flex-col">
            {atQuery !== null && filteredFiles.length > 0 && (
              <div className="absolute bottom-[calc(100%+8px)] left-0 right-0 bg-white rounded-12 shadow-[0_0_0_1px_theme(colors.neutral.200),theme(boxShadow.xl)] p-2 max-h-[260px] overflow-y-auto animate-[fadeUp_0.15s_ease]">
                <p className="text-subheading-md text-neutral-400 tracking-[0.06em] p-[4px_8px_6px]">
                  Files
                </p>
                    ...
                    {extractMentions(input).map((f) => (
                      ...
                      >
                        <span
                          ...
                          style={{ background: TYPE_BADGE[f.type] || '#737373' }}
                        />
                        <IconDocFile size={12} />
      ...
      {previewFile && (
        <DocumentCanvas
          file={{ ...previewFile, size: 'Unknown', date: 'Just now' }}
