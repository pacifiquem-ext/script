...
export function Footer() {
  return (
    <footer className="bg-white relative">
      <div className="container flex flex-col items-center text-center gap-8 pt-20 pb-8 max-sm:pt-12">
        ...
        <nav className="flex flex-wrap justify-center gap-x-6 gap-y-1 max-sm:gap-x-4">
          {FOOTER_LINKS.map((link) => (
            <Link
              key={link.label}
              to={link.href}
              className="text-neutral-600 no-underline py-1 transition-colors hover:text-neutral-950 text-para-sm"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            className="flex items-center justify-center w-9 h-9 text-neutral-400 rounded-8 border border-neutral-200 transition-all duration-200 hover:text-neutral-950 hover:bg-neutral-50 hover:border-neutral-300 hover:-rotate-6 hover:scale-110"
            aria-label="Twitter"
          >
            <svg
              width="15"
      ...
      <div
        className="flex justify-center leading-none overflow-hidden h-[clamp(63px,13.3vw,168px)] px-4 pointer-events-auto select-none"
        aria-hidden
      >
