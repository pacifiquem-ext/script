...
export function DocumentCanvas({
  ...
  const [documentContent, setDocumentContent] =
    useState(`This is the extracted text content for ${file.name}.
    ...
    <div className={`flex flex-col h-full bg-neutral-50 overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between p-[16px_20px] border-b border-neutral-200 shrink-0 bg-white">
        <div className="flex items-center gap-3 overflow-hidden">
          <span
            className="shrink-0 px-2 py-1 rounded-6 text-[10px] font-bold tracking-[0.05em] text-white"
            style={{ background: TYPE_COLOR[file.type] || '#737373' }}
          >
            {file.type.toUpperCase()}
          </span>
          <div>
            <p className="text-label-md text-neutral-950 whitespace-nowrap overflow-hidden text-ellipsis max-w-[240px]">
              {file.name}
            </p>
            <p className="text-para-xs text-neutral-400">
              {file.size} · {file.date}
            </p>
          </div>
        </div>
