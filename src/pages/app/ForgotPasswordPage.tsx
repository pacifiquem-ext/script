...
export function ForgotPasswordPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate('/app/verify-otp'); }, 800);
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-[24px_16px] bg-white relative overflow-hidden">
      ...
      <div className="w-full max-w-[400px] p-8 flex flex-col gap-6 relative z-20">
        ...
        <div className="text-center flex flex-col gap-1">
          ...
          <p className="text-para-sm text-neutral-600">Enter your email and we'll send you a code.</p>
        </div>
        ...
        <p className="text-para-sm text-neutral-600 text-center">
          Remembered it?{' '}
          <Link to="/app/login" className="text-primary-base font-medium no-underline transition-colors hover:text-primary-darker">Back to sign in</Link>
        </p>
      </div>
    </div>
  );
