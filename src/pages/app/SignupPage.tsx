...
export function SignupPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate('/app/signup-success'); }, 800);
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-[24px_16px] bg-white relative overflow-hidden">
      ...
      <div className="w-full max-w-[400px] p-8 flex flex-col gap-6 relative z-20">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <Button type="submit" size="md" loading={loading} className="self-center min-w-[200px] mt-2">
            Create account
          </Button>

          <p className="text-para-xs text-neutral-400 text-center mt-2">
            By signing up, you agree to our{' '}
            <a href="/terms" className="text-neutral-600 underline underline-offset-2 transition-colors hover:text-neutral-950">Terms of Service</a> and{' '}
            <a href="/privacy" className="text-neutral-600 underline underline-offset-2 transition-colors hover:text-neutral-950">Privacy Policy</a>.
          </p>
        </form>

        <p className="text-para-sm text-neutral-600 text-center">
          Already have an account?{' '}
          <Link to="/app/login" className="text-primary-base font-medium no-underline transition-colors hover:text-primary-darker">Sign in</Link>
        </p>
      </div>
    </div>
  );
